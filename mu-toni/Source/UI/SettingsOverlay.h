...
// mu-toni settings page. Shares the header bar / Close button / centred content column
// / group + section styling with mu-clid + mu-tant via mu_ui::SettingsOverlayBase, so
// every product's settings page looks identical. Content: master volume + UI size +
// transport BPM (General) and the standalone-only MIDI Clock rows (MIDI). The
// engine-specific rows (Note mode, Program Change) arrive with the engine + preset I/O.
