...
- **Modulator + gate + filter gate persistence** — each voice's `ControlSequence`s + `ModulationMatrix` + gate envelopes + filter envelopes serialised into `<VoiceData>` child of the APVTS state for full + `.muTant` presets, and into each `.muPattern`.
