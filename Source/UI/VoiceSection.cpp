...
void VoiceSection::wireCallbacks()
{
    ...
    struct { KnobWithLabel* k; const char* name; } entries[] = {
        { &pitchOctave,  "Pitch Octave"  }, { &pitchSemi,    "Pitch Semi"    },
        { &pitchFine,    "Pitch Fine"    }, { &pitchAtk,     "Pitch Atk"     },
        { &pitchDec,     "Pitch Dec"     }, { &pitchSus,     "Pitch Sus"     },
        { &pitchRel,     "Pitch Rel"     }, { &pitchDepth,   "Pitch Depth"   },
        { &filterCutoff, "Filter Cutoff" }, { &filterRes,    "Filter Res"    },
        { &filterAtk,    "Filt Env Atk"  }, { &filterDec,    "Filt Env Dec"  },
        { &filterSus,    "Filt Env Sus"  }, { &filterRel,    "Filt Env Rel"  },
        { &filterDepth,  "Filt Env Dep"  }, { &ampLevel,     "Amp Level"     },
        { &ampAtk,       "Amp Atk"       }, { &ampDec,       "Amp Dec"       },
        { &ampSus,       "Amp Sus"       }, { &ampRel,       "Amp Rel"       },
        { &driveDrive,   "Drive"         }, { &driveOutput,  "Drive Out"     },
    ...
    driveChar  .onChange       = [this](int id) { apvtsSet("drvChar", (float)(id - 1)); };
...
void VoiceSection::resized()
{
    ...
    const int kW = (w - 3 * divW) / 18;   // 18 cols, 3 dividers

    const int pitchX  = 0;
    const int filterX = 5 * kW + divW;
    const int ampX    = 10 * kW + 2 * divW;
    const int driveX  = 14 * kW + 3 * divW;
    ...
    pitchDepth.setBounds(pitchX + 4 * kW, row2Y, kW, rowH);

    // ─── Filter ──────────────────────────────────────────────────────────
    // Dropdown half-height centred; Cutoff+Res left-aligned in cols 1–2.
    ...
    filterDepth.setBounds(filterX + 4 * kW, row2Y, kW, rowH);
    ...
    // ─── Drive ───────────────────────────────────────────────────────────
    driveChar  .setBounds(driveX + 0 * kW, row1Y + rowH / 4, kW, rowH / 2);
    driveDrive .setBounds(driveX + 1 * kW, row1Y, kW, rowH);
    driveOutput.setBounds(driveX + 2 * kW, row1Y, kW, rowH);
    driveTone  .setBounds(driveX + 3 * kW, row1Y, kW, rowH);
}
...
void VoiceSection::paint(juce::Graphics& g)
{
    ...
    g.drawText("DRIVE",  14 * kW + 3 * divW,     0, 4 * kW, labelH, juce::Justification::centred, false);
}
