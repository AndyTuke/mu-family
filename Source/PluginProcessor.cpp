...
// #237 — forward declarations so saveRhythmPresetToFile / applyRhythmPreset / savePreset / loadPreset
// (defined earlier in this file) can reference the modulator serialise helpers defined below.
...
juce::AudioProcessorValueTreeState::ParameterLayout PluginProcessor::createParameterLayout()
{
    ...
    // Factories return unique_ptrs so they can be moved into a Group or directly
    // into the layout. IDs and names are unchanged from the pre-group layout —
    // groups add hierarchy for hosts that respect it (Cubase, Reaper, Bitwig,
    // FL) without altering automation lane identifiers or preset data.
    auto F = [](const juce::String& id, const juce::String& name, float mn, float mx, float def)
    {
    ...
    auto I = [](const juce::String& id, const juce::String& name, int mn, int mx, int def)
    {
        return std::make_unique<juce::AudioParameterInt>(id, name, mn, mx, def);
    };

    // #217: ADSR time params live in 0..10 seconds with a 0.3 skew. Skew 0.3 puts
    // ~100–200 ms at slider centre so the drum-snap region (1–30 ms) gets generous
    // resolution in the lower third of the knob, while pad/ambient values (1–10 s)
    // are reachable at the top. Stored value is in seconds directly so host
    // automation lanes display meaningful "0.150 s" / "2.5 s" instead of "0..100".
    // One-time preset-compat break vs the pre-#217 0..100 range; setStateInformation
    // detects legacy state via the kStateFormatVersion attribute and rescales.
    auto AdsrT = [](const juce::String& id, const juce::String& name, float def)
    {
        return std::make_unique<juce::AudioParameterFloat>(
    ...
    // One AudioProcessorParameterGroup per rhythm so hosts that respect parameter
    // hierarchy show eight collapsible folders ("Rhythm 1" … "Rhythm 8") instead
    // of 376 sibling automation destinations. Rhythms 0..kAutomatedRhythms-1
    // still use full "Rhythm N " names so flat-list hosts (Live, Logic) remain
    // readable; the rest use short "RN " names.
    for (int i = 0; i < SequencerEngine::MaxRhythms; ++i)
    {
        ...
        auto g = std::make_unique<Group>(
            "g_rhythm" + juce::String(i),
            "Rhythm " + juce::String(i + 1),
            "|");

        // HitGen A
        g->addChild(I(p+"stepsA",   n+"Steps A",   1, 64, 8));
        g->addChild(I(p+"hitsA",    n+"Hits A",    0, 64, 0));
        g->addChild(I(p+"rotA",     n+"Rot A",   -32, 32, 0));
        g->addChild(I(p+"prePadA",  n+"PrePad A",  0, 12, 0));
        g->addChild(I(p+"postPadA", n+"PostPad A", 0, 12, 0));
        g->addChild(I(p+"insStA",   n+"InsSt A",   0, 63, 0));
        g->addChild(I(p+"insLenA",  n+"InsLen A",  0,  8, 0));
        g->addChild(B(p+"insModeA",    n+"InsMode A",     false));
        g->addChild(B(p+"prePadModeA", n+"PrePadMode A",  false));
        g->addChild(B(p+"postPadModeA",n+"PostPadMode A", false));
        // HitGen B
        g->addChild(I(p+"stepsB",   n+"Steps B",   1, 64, 8));
        g->addChild(I(p+"hitsB",    n+"Hits B",    0, 64, 0));
        g->addChild(I(p+"rotB",     n+"Rot B",   -32, 32, 0));
        g->addChild(I(p+"prePadB",  n+"PrePad B",  0, 12, 0));
        g->addChild(I(p+"postPadB", n+"PostPad B", 0, 12, 0));
        g->addChild(I(p+"insStB",   n+"InsSt B",   0, 63, 0));
        g->addChild(I(p+"insLenB",  n+"InsLen B",  0,  8, 0));
        g->addChild(B(p+"insModeB",    n+"InsMode B",     false));
        g->addChild(B(p+"prePadModeB", n+"PrePadMode B",  false));
        g->addChild(B(p+"postPadModeB",n+"PostPadMode B", false));
        // HitGen C
        g->addChild(I(p+"stepsC",   n+"Steps C",   1, 64, 8));
        g->addChild(I(p+"hitsC",    n+"Hits C",    0, 64, 0));
        g->addChild(I(p+"rotC",     n+"Rot C",   -32, 32, 0));
        g->addChild(I(p+"prePadC",  n+"PrePad C",  0, 12, 0));
        g->addChild(I(p+"postPadC", n+"PostPad C", 0, 12, 0));
        g->addChild(I(p+"insStC",   n+"InsSt C",   0, 63, 0));
        g->addChild(I(p+"insLenC",  n+"InsLen C",  0,  8, 0));
        g->addChild(B(p+"insModeC",    n+"InsMode C",     false));
        g->addChild(B(p+"prePadModeC", n+"PrePadMode C",  false));
        g->addChild(B(p+"postPadModeC",n+"PostPadMode C", false));
        // Logic
        g->addChild(I(p+"logic", n+"Logic", 0, 4, 0));
        // Pitch
        g->addChild(I(p+"pitchOct",  n+"Pitch Oct",  -4,   4,  0));
        g->addChild(I(p+"pitchSemi", n+"Pitch Semi", -12,  12,  0));
        g->addChild(F(p+"pitchFine", n+"Pitch Fine", -100.0f, 100.0f, 0.0f));
        g->addChild(AdsrT(p+"pEnvAtk", n+"P Env Atk", 0.005f));   // #217 — seconds
        g->addChild(AdsrT(p+"pEnvDec", n+"P Env Dec", 0.1f));
        g->addChild(F(p+"pEnvSus",   n+"P Env Sus",  0.0f, 100.0f,  0.0f));  // sustain stays 0..100 %
        g->addChild(AdsrT(p+"pEnvRel", n+"P Env Rel", 0.1f));
        g->addChild(F(p+"pEnvDep",   n+"P Env Dep",  0.0f,  24.0f,  0.0f));
        g->addChild(B(p+"pEnvLeg",   n+"P Env Legato", false));   // #221
        // Filter
        g->addChild(I(p+"fltType", n+"Filter Type", 0, 15, 0));  // 0-15: LP12/HP12/BP12/Notch/LP24/HP24/BP24/LP6/Comb+/AP12/Notch24/HP6/Peak/LoShf/HiShf/Comb-
        // Log-skewed range: skew 0.25 puts ~1.3 kHz at slider centre and gives the
        // sub-bass / midrange the resolution they need. Otherwise 20–200 Hz lives
        // in 1% of knob travel.
        ...
        g->addChild(F(p+"fltRes",  n+"Filter Res",   0.0f,    0.99f,    0.2f));
        // Filter gain — only applied by Peak / LowShelf / HighShelf modes (types 12-14).
        // For other filter types this APVTS slot is dormant. Bipolar to allow cuts
        // as well as boosts; default 0 dB (no effect). Hidden in the UI for now —
        // accessible via host automation. See backlog #6 follow-up for UI surfacing.
        g->addChild(F(p+"fltGain", n+"Filter Gain",-18.0f,   18.0f,    0.0f));
        g->addChild(AdsrT(p+"fEnvAtk", n+"F Env Atk", 0.01f));   // #217 — seconds
        g->addChild(AdsrT(p+"fEnvDec", n+"F Env Dec", 0.3f));
        g->addChild(F(p+"fEnvSus", n+"F Env Sus",  0.0f, 100.0f,  0.0f));  // sustain stays 0..100 %
        g->addChild(AdsrT(p+"fEnvRel", n+"F Env Rel", 0.3f));
        // Bipolar so negative depth gives the inverted "closing" envelope shape
        // (attack closes the filter, decay reopens it) — a standard synth idiom.
        // Existing presets stored a normalised value in 0..1 mapping to 0..48; under
        // the new mapping that same normalised value maps to −48..+48 instead, so
        // legacy presets will load with a different effective depth.
        g->addChild(F(p+"fEnvDep", n+"F Env Dep", -48.0f, 48.0f, 0.0f));
        g->addChild(B(p+"fEnvLeg", n+"F Env Legato", false));   // #221
        // Amp
        g->addChild(F(p+"ampLvl",  n+"Amp Level",  0.0f,   2.0f,  1.0f));  // Issue #121: 0 dB default
        g->addChild(AdsrT(p+"aEnvAtk", n+"A Env Atk", 0.005f));   // #217 — seconds
        g->addChild(AdsrT(p+"aEnvDec", n+"A Env Dec", 0.3f));
        g->addChild(F(p+"aEnvSus", n+"A Env Sus",  0.0f, 100.0f, 80.0f));   // sustain stays 0..100 %
        g->addChild(AdsrT(p+"aEnvRel", n+"A Env Rel", 0.5f));
        g->addChild(B(p+"aEnvLeg",   n+"A Env Legato", false));   // #221
        g->addChild(F(p+"accentDb",  n+"Accent",     0.0f,  12.0f,  0.0f));
        // Insert effect. Display names are deliberately generic ("P1..P7") because
        // the same APVTS slot means very different things per character — e.g.
        // drvDrv is Drive on Soft/Hard/Fold/Tape, Threshold on Comp/Limit, Mix
        // on Ring Mod, Low Gain on EQ. The UI relabels per character via
        // configureInsertAlgorithm. Hosts that show only the APVTS display name
        // (most automation lanes) now read accurate generic text rather than
        // misleading character-specific names. IDs unchanged so saved automation
        // continues to match.
        g->addChild(I(p+"drvChar",    n+"Insert Type",  0,     10,      0));  // 0=None … 10=TapeSat
        g->addChild(F(p+"drvDrv",     n+"Insert P1",    0.0f, 100.0f,    0.0f));
        // Drive output / Compressor makeup gain. −12..+24 dB so the Compressor/Limiter
        // characters can actually recover the level lost to gain reduction; Soft/Hard/Fold
        // /Clipper/Tape Sat use this as a post-stage trim and benefit from the extra
        // headroom for parallel-distortion compensation. Default 0 dB keeps existing
        // presets' behaviour unchanged at unity.
        g->addChild(F(p+"drvOut",     n+"Insert P2",  -12.0f,   24.0f,   0.0f));
        g->addChild(F(p+"drvBits",    n+"Insert P3",    1.0f,  16.0f,   16.0f));
        g->addChild(F(p+"drvRate",    n+"Insert P4",  100.0f, 48000.0f, 48000.0f));
        g->addChild(B(p+"drvAa",      n+"Insert AA",   true));   // Bitcrush AA toggle (#205)
        g->addChild(F(p+"drvDit",     n+"Insert P5",    0.0f, 100.0f,    0.0f));
        g->addChild(F(p+"drvTon",     n+"Insert P6",   20.0f, 20000.0f, 20000.0f));
        g->addChild(F(p+"eqMidGain",  n+"Insert P7",  -18.0f,  18.0f,   0.0f));  // EQ mid-band gain (#129)

        layout.add(std::move(g));
    }

    // ── Effect slot (8 params) ────────────────────────────────────────────────
    // eff_algo: 0=Chorus, 1=Flanger, 2=Phaser, 3=Echo — the four registered
    // algorithms in FXAlgorithmRegistry::effectAlgorithms(). Range was 0..7
    // historically but values 4..7 silently mapped back to Chorus, so the
    // dropdown / automation lane is now tight to the actual set.
    {
        auto g = std::make_unique<Group>("g_effect", "Effect", "|");
        g->addChild(I("eff_algo", "Effect Algorithm", 0, 3, 0));
        g->addChild(B("eff_en",   "Effect Enable", true));
        // `eff_send`, `dly_send`, and `rev_lvl` were hidden cascaded gain stages
        // that doubled up with the FX-return strip faders in the mixer. Removed
        // so the return fader is the single user-controllable level for each FX
        // bus — matches the per-channel mixer behaviour. Legacy preset values
        // for those IDs are silently ignored on load.
        ...
        g->addChild(F("eff_p0", "Effect P0", 0.0f, 1.0f, 0.5f));
        g->addChild(F("eff_p1", "Effect P1", 0.0f, 1.0f, 0.5f));
        g->addChild(F("eff_p2", "Effect P2", 0.0f, 1.0f, 0.5f));
        g->addChild(F("eff_p3", "Effect P3", 0.0f, 1.0f, 0.5f));
        g->addChild(F("eff_p4", "Effect P4", 0.0f, 1.0f, 0.5f));
        layout.add(std::move(g));
    }

    // ── Delay slot (11 params) ────────────────────────────────────────────────
    {
        auto g = std::make_unique<Group>("g_delay", "Delay", "|");
        g->addChild(B("dly_en",        "Delay Enable",    true));
        g->addChild(B("dly_mode",      "Delay Sync",      false));
        g->addChild(F("dly_ms",        "Delay Time (ms)", 1.0f, 4000.0f, 250.0f));
        g->addChild(I("dly_syncDenom", "Delay Denom",     0, 3, 3));   // 0=1/32, 1=1/16, 2=1/8, 3=1/4
        g->addChild(B("dly_syncDot",   "Delay Dotted",    false));
        g->addChild(B("dly_syncTrip",  "Delay Triplet",   false));
        g->addChild(I("dly_count",     "Delay Count",     1, 8, 1));
        g->addChild(F("dly_fb",        "Delay Feedback",  0.0f,  0.98f, 0.45f));
        g->addChild(F("dly_spread",    "Delay Spread",    0.0f,  1.0f,  0.0f));
        g->addChild(F("dly_dirt",      "Delay Dirt",      0.0f,  1.0f,  0.0f));
        // Feedback-path LP damping: 0 = clean (filter wide open),
        // 1 = max tape-style HF decay on repeats. See DelaySlot::setDamp.
        g->addChild(F("dly_damp",      "Delay Damp",      0.0f,  1.0f,  0.0f));
        layout.add(std::move(g));
    }

    // ── Reverb slot (9 params) ────────────────────────────────────────────────
    {
        auto g = std::make_unique<Group>("g_reverb", "Reverb", "|");
        g->addChild(I("rev_algo", "Reverb Algorithm",  0, 3,  0));
        g->addChild(B("rev_en",   "Reverb Enable",     true));
        g->addChild(F("rev_size", "Reverb Size",   0.0f,   1.0f,  0.5f));
        g->addChild(F("rev_pre",  "Reverb Pre-Delay", 0.0f, 100.0f, 10.0f));
        g->addChild(F("rev_diff", "Reverb Diffusion", 0.0f,   1.0f,  0.7f));
        g->addChild(F("rev_damp", "Reverb Damp",   0.0f,   1.0f,  0.4f));
        g->addChild(F("rev_mod",  "Reverb Mod",    0.0f,   1.0f,  0.2f));
        g->addChild(F("rev_dirt", "Reverb Dirt",   0.0f,   1.0f,  0.0f));
        // rev_dirtPost: where to place the tanh saturation relative to the FDN
        // network. false (default) = pre-FDN, distortion goes into the room.
        // true = post-FDN, distortion colours the wet reverb tail itself
        // (industry-convention "vintage" position). Hidden in the UI; host
        // automation only — see issue #204.
        g->addChild(B("rev_dirtPost", "Reverb Dirt Post", false));
        layout.add(std::move(g));
    }

    // ── Intra-FX routing (3 params) ───────────────────────────────────────────
    {
        auto g = std::make_unique<Group>("g_intrafx", "Intra-FX", "|");
        g->addChild(F("eff2dly", juce::String::fromUTF8(u8"Effect→Delay"),  0.0f, 1.0f, 0.0f));
        g->addChild(F("eff2rev", juce::String::fromUTF8(u8"Effect→Reverb"), 0.0f, 1.0f, 0.0f));
        g->addChild(F("dly2rev", juce::String::fromUTF8(u8"Delay→Reverb"),  0.0f, 1.0f, 0.0f));
        layout.add(std::move(g));
    }

    // ── Echo (embedded in EFX slot when algo=Echo) ────────────────────────────
    {
        auto g = std::make_unique<Group>("g_echo", "Echo", "|");
        g->addChild(B("echo_en",        "Echo Enable",      true));
        g->addChild(F("echo_mode",      "Echo Mode",        0.0f, 1.0f, 0.0f));
        g->addChild(F("echo_ms",        "Echo Time Ms",     1.0f, 4000.0f, 250.0f));
        g->addChild(I("echo_syncDenom", "Echo Sync Denom",  0, 3, 2));
        g->addChild(B("echo_syncDot",   "Echo Dotted",      false));
        g->addChild(B("echo_syncTrip",  "Echo Triplet",     false));
        g->addChild(I("echo_count",     "Echo Count",       1, 8, 1));
        g->addChild(F("echo_fb",        "Echo Feedback",    0.0f, 1.0f, 0.45f));
        g->addChild(F("echo_spread",    "Echo Spread",      0.0f, 1.0f, 0.0f));
        g->addChild(F("echo_dirt",      "Echo Dirt",        0.0f, 1.0f, 0.0f));
        g->addChild(F("echo_damp",      "Echo Damp",        0.0f, 1.0f, 0.0f));   // feedback-path LP (#208)
        layout.add(std::move(g));
    }

    // ── Rhythm channel strips (12 × 8 = 96) ──────────────────────────────────
    // DAW-style fader: range 0..2 (= −∞..+6 dB), skewed so 0 dB (value 1.0) sits
    // at ~75% of slider travel. The bottom three quarters cover −∞..0 dB, the
    // top quarter is +0..+6 dB of boost.
    juce::NormalisableRange<float> chFaderRange(0.0f, MixerEngine::kFaderMax,
                                                 0.0f, MixerEngine::kFaderSkew);
    for (int i = 0; i < SequencerEngine::MaxRhythms; ++i)
    {
        ...
        auto g = std::make_unique<Group>(
            "g_chan" + juce::String(i),
            "Channel " + juce::String(i + 1),
            "|");

        g->addChild(std::make_unique<juce::AudioParameterFloat>(
            c+"lvl", n+"Level", chFaderRange, 1.0f));   // Issue #121: 0 dB default
        g->addChild(F(c+"pan",     n+"Pan",       -1.0f, 1.0f,  0.0f));
        g->addChild(B(c+"mute",    n+"Mute",      false));
        g->addChild(B(c+"solo",    n+"Solo",      false));
        g->addChild(F(c+"sendEff", n+"Send Eff",  0.0f, 1.0f,  0.0f));
        g->addChild(F(c+"sendDly", n+"Send Dly",  0.0f, 1.0f,  0.0f));
        g->addChild(F(c+"sendRev", n+"Send Rev",  0.0f, 1.0f,  0.0f));
        // Sidechain
        g->addChild(I(c+"scSrc",   n+"SC Src",    0, 8,     0));  // 0=off, 1-8=ch1-ch8
        g->addChild(F(c+"scAmt",   n+"SC Amount", 0.0f, 1.0f, 0.0f));
        g->addChild(F(c+"scAtk",   n+"SC Attack", 1.0f, 500.0f, 5.0f));
        g->addChild(F(c+"scRel",   n+"SC Release",10.0f, 2000.0f, 100.0f));
        // Multi-bus output routing: 0 = Master mix, 1..8 = direct out to Bus 1..8.
        g->addChild(I(c+"outBus",  n+"Output Bus",0, 8,     0));

        layout.add(std::move(g));
    }

    // ── Return channel strips (3 × 3 = 9) ────────────────────────────────────
    // Returns have mute but no solo: soloing an FX return has ambiguous
    // semantics (does it mute the dry channels too?), so the UI only exposes
    // solo on rhythm channels — see ReturnState in MixerEngine.h.
    {
        juce::NormalisableRange<float> faderRange(0.0f, MixerEngine::kFaderMax,
                                                  0.0f, MixerEngine::kFaderSkew);
        auto g = std::make_unique<Group>("g_returns", "Returns", "|");
        for (const char* ret : { "eff", "dly", "rev" })
        {
            const juce::String q = juce::String("ret_") + ret + "_";
            const juce::String nm = juce::String("Ret ") + ret + " ";
            g->addChild(std::make_unique<juce::AudioParameterFloat>(
                q+"lvl", nm+"Level", faderRange, 1.0f));  // Issue #121: 0 dB default
            g->addChild(F(q+"pan",  nm+"Pan",   -1.0f, 1.0f, 0.0f));
            g->addChild(B(q+"mute", nm+"Mute",  false));
        }
        layout.add(std::move(g));
    }

    // ── Master (2 params + 8 insert params) ──────────────────────────────────
    // Same DAW-style fader curve as channel/return faders.
    {
        auto g = std::make_unique<Group>("g_master", "Master", "|");
        g->addChild(std::make_unique<juce::AudioParameterFloat>(
            "mstr_lvl", "Master Level",
            juce::NormalisableRange<float>(0.0f, MixerEngine::kFaderMax, 0.0f, MixerEngine::kFaderSkew),
            1.0f));                                                          // Issue #121: 0 dB default
        g->addChild(F("mstr_pan",    "Master Pan",      -1.0f,    1.0f,     0.0f));
        g->addChild(I("mstrLoop",    "Master Loop",      0,       16,       0));      // 0=free, 1-16 → 16-256 steps
        // Master insert effect (#124): same algorithm set as per-rhythm voice INSERT.
        // Generic P1..P7 naming for the same reason as the per-rhythm Drive params
        // — each slot's meaning depends on the selected character.
        g->addChild(I("mst_insChar", "Mst Insert Type",  0,       10,       0));      // 0=None … 10=TapeSat
        g->addChild(F("mst_insDrv",  "Mst Insert P1",    0.0f,  100.0f,     0.0f));
        g->addChild(F("mst_insOut",  "Mst Insert P2",  -12.0f,   24.0f,     0.0f));
        g->addChild(F("mst_insBits", "Mst Insert P3",    1.0f,   16.0f,    16.0f));
        g->addChild(F("mst_insRate", "Mst Insert P4",  100.0f, 48000.0f, 48000.0f));
        g->addChild(B("mst_insAa",   "Mst Insert AA",  true));    // Bitcrush AA toggle (#205)
        g->addChild(F("mst_insDit",  "Mst Insert P5",    0.0f,  100.0f,     0.0f));
        g->addChild(F("mst_insTon",  "Mst Insert P6",   20.0f, 20000.0f, 20000.0f));
        g->addChild(F("mst_insMid",  "Mst Insert P7",  -18.0f,   18.0f,     0.0f));  // EQ-only: mid gain
        layout.add(std::move(g));
    }

#if MUCLID_LITE_BUILD
    layout.add(I("lite_midiNote",   "MIDI Note", 0, 127, 36));
    layout.add(F("lite_accentAmt",  "Accent",    0.0f, 100.0f, 0.0f));
#endif
      ...
      apvts(*this, nullptr, "MuClidState", createParameterLayout())
{
    ...
    // pitch.octave / pitch.fine deprecated by #218 — removed from the populated set so
    // legacy preset assignments targeting them resolve to "destination not found" at
    // apply time (paramValues.find returns end() → assignment silently no-ops).
                                   ...
                                   juce::MidiBuffer& midiMessages)
{
    ...
    // #231: detect transport stop→start edge and reset the sequencer's wrap
    // detector so the first block after restart can't emit a false `masterLoopWrapped`
    // from a stale `lastEffectiveStep`.
    {
        ...
        if (!rhythm.modulationMatrix.getAssignments().empty())
        {
            ...
            if (rhythm.modLock.compare_exchange_strong(expected, true, std::memory_order_acquire))
            {
                ...
                // Snapshot pre-normalised values for the UI live-arc indicator (#133).
                {
                    ...
                    snap[kSnapFenvDepth]   .set(sn(modParamValues["fenv.depth"],     -48.0f,    48.0f));  // bipolar per #214
                    ...
                    snap[kSnapInsLpf]      .set(snLogHz(modParamValues["insert.lpf"]));
                    // #223 new destinations
    ...
    mixerEngine.processBlock(masterBus, numRhythms,
                             voiceEngines.data(),
                             tailVoices.data(),
                             fxChain, buffer.getNumSamples(),
                             &directPtrs, fxRetPtr);

    // Edge-detect: any tail voice that has finished producing sound triggers a
    // single async cleanup. The unique_ptr is released on the message thread
    // (handleAsyncUpdate) under rhythmsLock — never on the audio thread.
    bool needsTailReap = false;
    for (int r = 0; r < numRhythms; ++r)
    {
        if (tailVoices[r]
            && !tailVoices[r]->isProducingSound()
            && !tailReaperPending[r].load(std::memory_order_relaxed))
        {
            tailReaperPending[r].store(true, std::memory_order_release);
            needsTailReap = true;
        }
    }
    if (needsTailReap) triggerAsyncUpdate();
...
// #217: ADSR A/D/R values are stored in seconds directly in APVTS now (0..10 s, skew 0.3).
// Helper is now just a floor at 0.001 s to avoid `pow(0, …)` zero-divides downstream.
static inline float adsrTime(float v) { return juce::jmax(0.001f, v); }
// Convert 0–100 UI scale → 0–1 amplitude for ADSR Sustain. Sustain stays a percentage.
...
static const char* const kRhythmSuffixes[] = {
    ...
    "fltType","fltCut","fltRes","fltGain","fEnvAtk","fEnvDec","fEnvSus","fEnvRel","fEnvDep","fEnvLeg",
    "ampLvl","aEnvAtk","aEnvDec","aEnvSus","aEnvRel","aEnvLeg","accentDb",
    "drvChar","drvDrv","drvOut","drvBits","drvRate","drvAa","drvDit","drvTon","eqMidGain",
    nullptr
};
...
static const char* const kGlobalParams[] = {
    "eff_algo","eff_en","eff_p0","eff_p1","eff_p2","eff_p3","eff_p4",
    "dly_en","dly_mode","dly_ms","dly_syncDenom","dly_syncDot","dly_syncTrip",
    "dly_count","dly_fb","dly_spread","dly_dirt","dly_damp",
    "rev_algo","rev_en","rev_size","rev_pre","rev_diff","rev_damp","rev_mod","rev_dirt","rev_dirtPost",
    "eff2dly","eff2rev","dly2rev",
    "echo_en","echo_mode","echo_ms","echo_syncDenom","echo_syncDot","echo_syncTrip",
    "echo_count","echo_fb","echo_spread","echo_dirt","echo_damp",
    "ret_eff_lvl","ret_eff_pan","ret_eff_mute",
    "ret_dly_lvl","ret_dly_pan","ret_dly_mute",
    "ret_rev_lvl","ret_rev_pan","ret_rev_mute",
    "mstr_lvl","mstr_pan","mstrLoop",
    "mst_insChar","mst_insDrv","mst_insOut","mst_insBits","mst_insRate","mst_insAa","mst_insDit","mst_insTon","mst_insMid",
    nullptr
};
                               ...
                               bool& patternDirty, bool& voiceDirty)
{
    ...
    else if (suffix == "fltGain")   { r.voiceParams.filterGainDb   = v;            voiceDirty = true; }
    ...
    else if (suffix == "aEnvRel")   { r.voiceParams.ampEnvRel = adsrTime(v); r.voiceParams.ampRelToEnd = (v >= 10.0f); voiceDirty = true; }   // #217: End threshold = new max
    ...
    else if (suffix == "drvAa")      { r.voiceParams.drvAa      = v > 0.5f; voiceDirty = true; }
...
void PluginProcessor::syncFXParam(const juce::String& id, float v)
{
    ...
    if      (id == "eff_algo")
    {
        if (!apvtsLoading)
        {
            eff.setAlgorithm((int)v);
            // After swapping algorithm, the new algorithm starts with its
            // built-in defaults — push the current eff_p0..p4 APVTS values back
            // through so the engine reflects what the user/preset has stored.
            // Without this, the UI and engine disagree until the user touches
            // each knob individually (which fires the listener again).
            ...
            if (ai < (int)algos.size())
            {
                ...
                {
                    if (auto* raw = apvts.getRawParameterValue("eff_p" + juce::String(i)))
                    {
                        const float norm   = *raw;
                        const auto& pd     = params[i];
                        eff.setParam(pd.id, pd.minVal + norm * (pd.maxVal - pd.minVal));
                    }
                }
            }
        }
    ...
    else if (id == "dly_damp")   { dly.setDamp(v); }
    ...
    else if (id == "rev_dirtPost") { rev.setDirtPost(v > 0.5f); }
    ...
    else if (id == "echo_damp")   { eff.getEchoDelay().setDamp(v); }
}
...
void PluginProcessor::syncMixerParam(const juce::String& id, float v)
{
    ...
    if (id.startsWith("ret_"))
    {
        ...
        if (retIdx >= 0)
        {
            ...
            // "solo" intentionally absent — FX returns no longer expose solo.
            // Legacy preset values for ret_*_solo are silently ignored.
        }
        return;
    }
    ...
    else if (id == "mst_insAa")   mixerEngine.masterInsertParams.drvAa      = v > 0.5f;
...
void PluginProcessor::pushRhythmToAPVTS(int ri)
{
    ...
    // #217: ADSR A/D/R stored in APVTS as seconds directly (0..10); sustain stays as 0..100 %.
    ...
    set(px+"fltGain",   vp.filterGainDb);
    ...
    set(px+"aEnvAtk",   vp.ampEnvAtk);
    set(px+"aEnvDec",   vp.ampEnvDec);
    set(px+"aEnvSus",   vp.ampEnvSus  * 100.0f);
    // ampRel: when ampRelToEnd is on, the legacy convention pushed 100 (= old max);
    // new max is 10 s, so write 10 to signal "End" via the >= 10 check in applyRhythmSuffix.
    ...
    set(px+"drvAa",     vp.drvAa ? 1.0f : 0.0f);
...
void PluginProcessor::addRhythm(const Rhythm& r)
{
    ...
    // Reset the freshly activated mixer channel to defaults so it can't
    // inherit stale state left behind by a prior removeRhythm at this slot.
    // pushMixerChannelToAPVTS below mirrors that to APVTS so any leftover
    // values in the parameter tree are overwritten too.
    mixerEngine.channels[ri] = MixerEngine::ChannelState{};
    mixerEngine.resetSidechainEnv(ri);
...
bool PluginProcessor::swapRhythms(int i, int j)
{
    ...
    // bail (silent block) while we hold the lock — clean swap atomically.
    suspendProcessing(true);
    {
        ...
        std::swap(tailVoices [i], tailVoices [j]);   // tail follows its rhythm to the new slot
...
void PluginProcessor::removeRhythm(int index)
{
    ...
    suspendProcessing(true);
    {
        ...
        // Drop any tail voice on the slot being removed, then shift the
        // remaining voices/tails/midi/mixer state down to close the gap.
        tailVoices[index].reset();
        tailReaperPending[index].store(false, std::memory_order_relaxed);
        for (int i = index; i < newN; ++i)
        {
            voiceEngines[i] = std::move(voiceEngines[i + 1]);
            tailVoices  [i] = std::move(tailVoices  [i + 1]);
            tailReaperPending[i].store(
                tailReaperPending[i + 1].load(std::memory_order_relaxed),
                std::memory_order_relaxed);
            midiEngines[i]  = std::move(midiEngines[i + 1]);
            mixerEngine.channels[i] = mixerEngine.channels[i + 1];
        }
        voiceEngines[newN].reset();
        tailVoices  [newN].reset();
        tailReaperPending[newN].store(false, std::memory_order_relaxed);
        midiEngines[newN] = MidiOutputEngine{};
        mixerEngine.channels[newN] = MixerEngine::ChannelState{};

        // Re-translate sidechain source indices to follow the shift, mirroring
        // the logic in swapRhythms: a channel sidechained off the removed slot
        // gets turned off; sources above the removed index shift down by one
        // so they keep pointing at the same logical rhythm.
        for (int c = 0; c < newN; ++c)
        {
            auto& src = mixerEngine.channels[c].sidechainSource;
            if      (src == index) src = -1;
            else if (src >  index) src -= 1;
        }
        mixerEngine.resetSidechainEnv(newN);
    }
    suspendProcessing(false);

    // Mirror the shifted engine state back into APVTS — otherwise the parameter
    // tree retains the pre-shift values, presets save the wrong channels, and
    // the next listener callback overwrites the engine back to stale data.
    // Push includes the now-vacant slot at index newN so a subsequent
    // addRhythm starts from a clean parameter tree.
    apvtsLoading = true;
    for (int i = 0; i <= newN && i < SequencerEngine::MaxRhythms; ++i)
...
void PluginProcessor::handleAsyncUpdate()
{
    ...
    for (int r = 0; r < n; ++r)
    {
        ...
        // Hold rhythmsLock for the actual swap — processBlock's ScopedTryLock at
        // the top will fail and output silence for the one block during which the
        // lock is held, ensuring the audio thread cannot read voiceEngines[r] mid-
        // destruction. Old voice is demoted to tailVoices[r] (any previous tail
        // is destroyed here — only the most recent transition can carry a tail).
        // The audio path sums active + tail into the same channel buffer so they
        // share the mixer state; the audio thread signals when the tail goes
        // silent and we reap it below.
        {
            const juce::ScopedLock sl(rhythmsLock);
            tailVoices[r]   = std::move(voiceEngines[r]);
            voiceEngines[r] = std::move(sw.pendingVoice);
            tailReaperPending[r].store(false, std::memory_order_relaxed);
        ...
        // Bump the swap epoch so UI components refresh their name/colour caches
        // (those fields aren't APVTS-backed and won't propagate via the listener).
        rhythmSwapEpoch.fetch_add(1, std::memory_order_release);
    }

    // Reap any tail voices the audio thread flagged as finished. We release the
    // unique_ptr under rhythmsLock so the audio thread can't read a half-destroyed
    // object. Audio thread sets the flag exactly once on the silent edge.
    for (int r = 0; r < (int)tailVoices.size(); ++r)
    {
        if (!tailReaperPending[r].load(std::memory_order_acquire)) continue;
        const juce::ScopedLock sl(rhythmsLock);
        tailVoices[r].reset();
        tailReaperPending[r].store(false, std::memory_order_relaxed);
    }
                                             ...
                                             bool embedSample)
{
    ...
    state.addChild(serialiseModulators(r), -1, nullptr);
...
bool PluginProcessor::applyRhythmPreset(const juce::File& file, int targetIdx)
{
    ...
    // #237: restore modulators if the preset includes them. clearModulators() first
    // so we don't accumulate assignments from the previous rhythm.
    clearModulators(r);
    auto mods = state.getChildWithName("Modulators");
    if (mods.isValid())
...
//==============================================================================
// #237 — Modulator state serialisation. Captures everything not APVTS-backed:
// per-rhythm ControlSequences (mode/polarity/loop+step timing/stepValues/curvePoints
// with Bézier handles) and ModulationMatrix assignments (id/source/dest/depth/curve).
// Schema (one child per rhythm under an enclosing "Modulators" tree):
//   <Modulators>
...
static void deserialiseModulators(const juce::ValueTree& mods, Rhythm& r)
{
    ...
    // Spin on the rhythm's modLock so the audio thread can't be inside process()
    // while we overwrite the CS/matrix vectors. Mirrors ModulatorEditor's lock path.
    ...
    for (int ci = 0; ci < mods.getNumChildren(); ++ci)
    {
        ...
        if (node.getType() == juce::Identifier("Seq"))
        {
            ...
            // Find the matching CS by id (csN). The vector is pre-sized to MaxControlSequences
            // in Rhythm's ctor so we just patch the slot in place.
...
// Clears all CS step/curve data + assignments back to defaults (matches Rhythm ctor
// state). Used before deserialising so a freshly-loaded preset starts clean — without
// this, assignments would accumulate across loads.
static void clearModulators(Rhythm& r)
{
    while (r.modLock.exchange(true, std::memory_order_acquire))
        std::this_thread::yield();

    // Drop all existing assignments. ModulationMatrix has no clear() so we iterate.
                              ...
                              SequencerEngine& seq, const juce::StringArray& samplePaths)
{
    ...
    for (int i = 0; i < numRhythms; ++i)
    {
        ...
        // #237: per-rhythm modulator state as a child named "Mods_r{i}".
...
// #217: bump on any APVTS-unit change that needs load-time migration. Pre-versioned
// state (no attribute, or < kCurrentStateFormatVersion) gets rescaled in setStateInformation.
//   v2 (#217): ADSR A/D/R params migrated from 0..100 → 0..10 seconds (× 0.03).
static constexpr int kCurrentStateFormatVersion = 2;

void PluginProcessor::getStateInformation(juce::MemoryBlock& destData)
{
    auto state = apvts.copyState();
    state.setProperty("muclidStateFmt", kCurrentStateFormatVersion, nullptr);
...
// #217 migration helper: walk APVTS XML, find PARAM elements whose id ends in
// EnvAtk/EnvDec/EnvRel (pitch/filter/amp), and rescale their stored value from the
// legacy 0..100 range (× 0.03) into the new 0..10 seconds range. Sustain params
// (EnvSus) stay 0..100 % and are left alone. Idempotent guard: only runs when the
// state predates kCurrentStateFormatVersion.
static void migrateAdsrTimes_0to100_to_0to10(juce::XmlElement& root)
{
    for (auto* el : root.getChildIterator())
    {
        if (el->hasTagName("PARAM"))
        {
            ...
            if ((id.endsWith("EnvAtk") || id.endsWith("EnvDec") || id.endsWith("EnvRel"))
                && el->hasAttribute("value"))
            {
                const float legacy = (float)el->getDoubleAttribute("value");
                el->setAttribute("value", juce::jlimit(0.0f, 10.0f, legacy * 0.03f));
            }
        }
        else
        {
            migrateAdsrTimes_0to100_to_0to10(*el);
        }
    }
}
...
void PluginProcessor::setStateInformation(const void* data, int sizeInBytes)
{
    ...
    {
        // #217: rescale legacy ADSR time values (0..100 → 0..10 s) before APVTS load.
        const int storedVersion = xml->getIntAttribute("muclidStateFmt", 1);
        if (storedVersion < kCurrentStateFormatVersion)
            migrateAdsrTimes_0to100_to_0to10(*xml);
    ...
    if (root.getType() == juce::Identifier("MuClidPreset"))
    {
        ...
        if (n < oldN2)
        {
            ...
            for (int i = n; i < oldN2; ++i)
            {
                voiceEngines[i].reset();
                tailVoices  [i].reset();
                tailReaperPending[i].store(false, std::memory_order_relaxed);
                midiEngines[i] = MidiOutputEngine{};
        ...
        for (int ci = 0; ci < root.getNumChildren() && rhythmIdx < n; ++ci)
        {
            ...
            // #237: restore modulators if present.
            clearModulators(r);
