...
| 23 | ✅ Done | **Hot-swap rhythm while playing** — "R.Pst" button in rhythm panel header opens `.muRhyth` file chooser; if not playing, applies immediately; if playing, stages for atomic commit at the next loop boundary. SequencerEngine `BlockResult` extended with `rhythmLoopWrapMask` and `masterLoopWrapped`; PluginProcessor adds `PendingRhythmSwap` struct array, `AsyncUpdater` base, and `stageRhythmPreset`/`cancelStagedSwap`/`hasPendingSwap`; audio thread sets `boundaryReached` flag, message thread (via `handleAsyncUpdate`) does `suspendProcessing` + VoiceEngine swap + APVTS sync. Orange "SWP" badge in sidebar indicates pending swap; click cancels it. "Hot-swap timing" dropdown in Settings selects On master loop vs On rhythm loop. | #97 |
