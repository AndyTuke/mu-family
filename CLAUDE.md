...
Stages 1–26 are complete. See [docs/DevelopmentHistory.md](docs/DevelopmentHistory.md) for the full stage-by-stage log with dates. When a stage is completed, it should be moved to the DevelopmentHistory.md file.
...
| 27 | Pending | **Ring modulator INSERT** — add Ring Modulator as a new per-rhythm voice INSERT algorithm (driveChar=9), sitting in the same slot as Soft/Hard/Fold/Bitcrusher/Clipper/EQ/Compressor/Limiter. DSP: multiply the input signal sample-by-sample by a sine wave at a user-set carrier frequency. Parameters: **Frequency** (10–5000 Hz, log scale), **Mix** (0–100% wet/dry). Creates metallic/clangorous tones from percussive samples. Reuse existing APVTS params: `drvTon` → carrier frequency (20–20000 Hz range already present), `drvDrv` → mix (0–100). UI: VoiceSection `configureInsertAlgorithm` case 9; label "Freq" for drvTon knob, "Mix" for drvDrv. Also available on master insert (#124) as `mst_insChar=9`. ModDest: no new targets needed. | — |
| 28 | Pending | **Tape saturation INSERT** — add Tape Saturation as a new per-rhythm voice INSERT algorithm (driveChar=10). Use a simplified tape model: input gain → soft-knee tanh saturation curve → high-frequency roll-off → output trim. Parameters: **Drive** (0–100% input gain), **Tone** (post-saturation LPF cutoff, 20–20 kHz), **Output** (makeup trim, −24–0 dB). Adds warmth and subtle compression rather than hard clipping. Reuses existing APVTS params: `drvDrv` (drive), `drvTon` (tone/LPF), `drvOut` (output). UI: `configureInsertAlgorithm` case 10; labels Drive / Tone / Output. Also available on master insert as `mst_insChar=10`. | — |
