...
| 35 | ✅ Done | Robust preset format. (1) ✅ Renamed `VoiceParams::drive*` / `drv*` C++ fields → `insert*` (#452). APVTS IDs + preset XML keys stay as `drv*` for back-compat — DAW automation lanes and v0/v1/v2 preset files all keep working. (2) ✅ De-normalised preset save format — `writeKindedProperty` writes actual values (`r0_stepsA="16"`, `r0_fltCut="234.7"`) instead of JUCE-normalised 0..1. (3) ✅ Algorithm selectors written as **string names** (`r0_drvChar="Bitcrusher"`, `r0_fltType="LP24"`, `eff_algo="chorus"`, modulator `mode="Smooth"`) via [Source/Audio/AlgorithmNames.h](Source/Audio/AlgorithmNames.h) lookup tables — reordering / inserting / removing algorithms preserves saved presets. Covers per-rhythm params, global FX/mixer state (#451), modulator enums (#436). (4) ✅ `presetVersion` bumped to 2; v0 / v1 / v2 all load via `readKindedPropertyAsActualV2` with appropriate per-version handling. (5) ✅ Documentation in [docs/preset-format.md](docs/preset-format.md). Should be moved to DevelopmentHistory next stage start. | #447-#452 |
