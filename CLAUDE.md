...
| 35 | 🟢 Mostly complete | Robust preset format. (1) ⏸ Rename `drv*` APVTS IDs + `VoiceParams.drive*` members to `ins*` / `insert*` — **deferred** as a Stage 35 follow-up (549 refs across 20 files, win is cosmetic, risk too high without round-trip test coverage). (2) ✅ De-normalised preset save format — `writeParamPropertyV2` writes actual values (`r0_stepsA="16"`, `r0_fltCut="234.7"`) instead of JUCE-normalised 0..1. (3) ✅ Algorithm selectors written as **string names** (`r0_drvChar="Bitcrusher"`, `r0_fltType="LP24"`) via [Source/Audio/AlgorithmNames.h](Source/Audio/AlgorithmNames.h) lookup tables — reordering / inserting / removing algorithms preserves saved presets. (4) ✅ `presetVersion` bumped to 2; v0 / v1 / v2 all load via `readParamPropertyAsActual` with appropriate per-version handling. (5) ✅ Documentation in [docs/preset-format.md](docs/preset-format.md). Step-by-step plan in [docs/design-stage35-preset-format.md](docs/design-stage35-preset-format.md). | #447-#451 (allocated below) |
