...
| Confirmation prompts | Shared `mu-core/UI/ConfirmDialog.h` | `mu_ui::confirmAsync` (reset/delete) + `mu_ui::confirmQuitAsync` (OK/Save/Cancel quit). Use these so reset/delete/quit prompts read identically across products; do NOT build a one-off AlertWindow. |
