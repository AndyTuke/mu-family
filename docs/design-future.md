...
### New effect algorithms (Stages 25–28)

**Stage 25 — Chorus** (Signalsmith-dsp): Chorus as a new Effect slot algorithm. Signalsmith-dsp (already a project dependency) provides a stereo chorus with clean anti-aliased modulation. Parameters: Rate (LFO speed in Hz), Depth (modulation depth in ms), Mix (wet/dry). Integration path: new `ChorusEffect : FXSlotBase` class alongside existing `FlangerEffect`; add algorithm entry to `FXAlgorithmRegistry::effectAlgorithms()`. APVTS params: `eff_choRate`, `eff_choDep`, `eff_choMix`.

**Stage 26 — Multimode SVF filter** (Martin Vicanek TPT-SVF): Replace the one-pole LPF tone filter in the voice chain with a State Variable Filter giving LP / HP / BP / Notch modes. The Vicanek TPT design is topology-preserving (no frequency warping artifacts) and numerically stable at high resonance. UI: Mode dropdown (LP / HP / BP / Notch) + Resonance knob added to the filter config row in VoiceSection. APVTS: `r{N}_filterMode` (int 0–3), `r{N}_filterRes` (0–1). Modulation: `filterRes` becomes a valid modulation target alongside `filterCutoff`. Implementation: `Source/Audio/AudioFilters.h` — add `TPT_SVF` struct with `prepare(cutoff, res, sr)` and `process(x)` per channel, matching the existing `OnePoleLP` pattern.

**Stage 27 — Ring modulator** (self-contained): Ring Modulator as a new Effect slot algorithm. Implementation: multiply input signal by `sin(2π × freq × t)` per sample — one oscillator phase accumulator, no library needed. Parameters: Frequency (10–5000 Hz, log-scaled knob), Mix (wet/dry). Creates metallic, bell-like, or robotic tones from percussive samples. New file: `Source/FX/Effects/RingModEffect.h`. APVTS params: `eff_rmFreq`, `eff_rmMix`.

**Stage 28 — Tape saturation** (ChowDSP or simplified): Tape Saturation as a new Effect slot algorithm. Options: (a) integrate Chowdhury DSP's CHOWTapeModel (MIT) as a submodule, or (b) implement a simplified version: input gain → soft-knee tanh saturation → high-frequency roll-off (one-pole LPF) → output trim. The simplified version requires no extra dependency and captures the essential warmth/compression character. Parameters: Drive (pre-gain, 0–100%), Bias (DC offset before saturation — shifts even/odd harmonic balance), Tone (post LPF cutoff), Mix. New file: `Source/FX/Effects/TapeEffect.h`. APVTS params: `eff_tapeDrive`, `eff_tapeBias`, `eff_tapeTone`, `eff_tapeMix`.
