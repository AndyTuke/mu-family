...
| **L6** ✅ | **Mixing + robustness.** Per-client level/pan/mute/solo + master; heartbeat-based client reaping (reap a client that died without detaching); MIDI-clock-out to outboard gear validated. | **Done (v1.0.780, #901).** Per-client **gain / pan (linear balance) / mute / solo + master gain** implemented **directly in `ServerEngine`** (lightweight, not the plugin-coupled `MixerEngine` — documented deviation) and exposed as GUI knob + M/S toggles per strip. **Heartbeat reaping:** the engine reaps a slot whose heartbeat freezes for >2 s (client crashed without detaching) so its zombie ring/meter clears and the slot frees. 27/27 tests (gain scales, mute silences, solo isolates, zombie reaped, live client untouched). Plus a **master safety soft-clip** (#903) so the summed bus can never hard-clip. MIDI-clock-out to outboard gear is wired (L3 picker → `MidiClockOut`) but final validation needs hardware (manual). **mu-tant standalone wired too (#902)** — the bridge + playhead were promoted to the shared header-only `mu-core/Link/MuLinkBridge.h` and `HostTransport` gained a position field, so every product slaves to mu-link via one family standard (not a per-product hack). |
| **L7** ✅ | **External MIDI-clock sync (slave mode).** mu-link can slave its master clock to incoming MIDI clock (critical for a performance tool), via a manual Internal / Ext-MIDI toggle, without inheriting MIDI jitter. | **Done (v1.0.781, #904).** Pure `MidiClockEstimator` (tempo PLL: F8 interval → one-pole-smoothed BPM, jitter-rejected + range-gated; FA/FB/FC transport; Start→reset) — unit-tested headless (tracks 120→140 BPM, transport edges). `MidiClockInput` adapts a JUCE MIDI-input callback to it (monotonic hi-res timestamps). `ServerEngine` gained a `ClockSource` (Internal/ExternalMidi); in external mode it follows the estimator's tempo + transport each block while the frame counter stays the timebase. `AudioServer` registers the MIDI-in callback on the device manager; GUI got a **Clock: Internal / Ext MIDI** toggle + MIDI-input enabled in the picker (external mode shows the detected BPM read-only + locks the local tempo/play). 29/29 tests. |
